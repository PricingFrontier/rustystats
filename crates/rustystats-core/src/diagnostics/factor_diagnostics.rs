...
// =============================================================================
// Fast Categorical Residual Pattern
// =============================================================================

/// Result for categorical residual pattern (eta-squared)
#[derive(Debug, Clone)]
pub struct CategoricalResidualResult {
    pub mean_abs_residual: f64,
    pub eta_squared: f64,
}

/// Compute residual pattern for categorical factor - O(n) with HashMap
pub fn compute_categorical_residual_pattern(
    factor_values: &[String],
    residuals: &[f64],
) -> CategoricalResidualResult {
    let n = factor_values.len();
    if n == 0 || n != residuals.len() {
        return CategoricalResidualResult {
            mean_abs_residual: 0.0,
            eta_squared: 0.0,
        };
    }
    
    // Single pass to compute overall mean
    let overall_sum: f64 = residuals.iter().sum();
    let overall_mean = overall_sum / n as f64;
    
    // Single pass groupby using HashMap
    let mut level_data: HashMap<&str, (f64, usize)> = HashMap::new();
    
    for (i, level) in factor_values.iter().enumerate() {
        let entry = level_data.entry(level.as_str()).or_insert((0.0, 0));
        entry.0 += residuals[i];
        entry.1 += 1;
    }
    
    // Compute level means and eta-squared
    let mut ss_between = 0.0;
    let mut sum_abs_means = 0.0;
    
    for (_, (sum, count)) in &level_data {
        let level_mean = sum / *count as f64;
        ss_between += *count as f64 * (level_mean - overall_mean).powi(2);
        sum_abs_means += level_mean.abs();
    }
    ...
    let eta_squared = if ss_total > 0.0 { ss_between / ss_total } else { 0.0 };
    let mean_abs_residual = sum_abs_means / level_data.len() as f64;
...
// =============================================================================
// Partial Dependence (Fast Categorical Aggregation)
// =============================================================================

/// Result for categorical partial dependence
#[derive(Debug, Clone)]
pub struct CategoricalPDResult {
    pub levels: Vec<String>,
    pub mean_predictions: Vec<f64>,
    pub counts: Vec<usize>,
    pub relativities: Vec<f64>,
}

/// Compute categorical partial dependence via fast groupby
/// Returns mean prediction for each level - O(n) with HashMap
pub fn compute_categorical_pd(
    factor_values: &[String],
    predictions: &[f64],
) -> CategoricalPDResult {
    let n = factor_values.len();
    if n == 0 || n != predictions.len() {
        return CategoricalPDResult {
            levels: Vec::new(),
            mean_predictions: Vec::new(),
            counts: Vec::new(),
            relativities: Vec::new(),
        };
    }
    
    // Single pass aggregation using HashMap
    ...
    // Convert to vectors
    let mut results: Vec<(&str, f64, usize)> = level_sums.iter()
        .map(|(&level, &(sum, count))| (level, sum / count as f64, count))
        .collect();
    
    // Sort by count (descending) for consistent ordering
    results.sort_by(|a, b| b.2.cmp(&a.2));
    
    let levels: Vec<String> = results.iter().map(|(l, _, _)| l.to_string()).collect();
    let mean_predictions: Vec<f64> = results.iter().map(|(_, m, _)| *m).collect();
    let counts: Vec<usize> = results.iter().map(|(_, _, c)| *c).collect();
    
    // Compute relativities (first level as base)
    let base = mean_predictions.first().copied().unwrap_or(1.0);
    let relativities: Vec<f64> = mean_predictions.iter()
        .map(|&p| if base > 0.0 { p / base } else { 1.0 })
        .collect();
...
// =============================================================================
// Batch Factor Diagnostics (All factors in one pass)
// =============================================================================

/// Result for a single factor's A/E summary
#[derive(Debug, Clone)]
pub struct FactorAESummary {
    pub factor_name: String,
    pub is_categorical: bool,
    pub n_levels: usize,
    pub ae_range: (f64, f64),  // (min, max) A/E
    pub worst_ae: f64,
    pub worst_level: String,
    pub has_problem: bool,
}

/// Batch compute A/E summaries for multiple categorical factors in one pass
/// More memory efficient than computing each separately
pub fn batch_compute_ae_categorical(
    factor_names: &[String],
    ...
    let mut results = Vec::with_capacity(factor_names.len());
    
    for (idx, factor_name) in factor_names.iter().enumerate() {
        let values = match factor_values.get(idx) {
            Some(v) if v.len() == n => v,
            _ => continue,
        };
        
        // Single pass aggregation
        let mut level_data: HashMap<&str, (f64, f64)> = HashMap::new();
        
        for (i, level) in values.iter().enumerate() {
            let entry = level_data.entry(level.as_str()).or_insert((0.0, 0.0));
            entry.0 += y[i];   // actual
            entry.1 += mu[i];  // predicted
        }
        
        // Compute A/E for each level
        let mut min_ae = f64::MAX;
        let mut max_ae = f64::MIN;
        let mut worst_ae = 1.0;
        let mut worst_level = String::new();
        let mut has_problem = false;
        
        for (&level, &(actual, predicted)) in &level_data {
            let ae = if predicted > 0.0 { actual / predicted } else { f64::NAN };
            if ae.is_finite() {
                min_ae = min_ae.min(ae);
                max_ae = max_ae.max(ae);
                
                let deviation: f64 = (ae - 1.0).abs();
                if deviation > (worst_ae - 1.0_f64).abs() {
                    worst_ae = ae;
                    worst_level = level.to_string();
                }
        ...
        results.push(FactorAESummary {
            factor_name: factor_name.clone(),
            is_categorical: true,
            n_levels: level_data.len(),
            ae_range: (min_ae, max_ae),
            worst_ae,
            worst_level,
            has_problem,
        });
