...
pub fn compute_knots_natural(
    ...
    let n_interior = df.saturating_sub(1);
...
fn bspline_all_basis_at_point(x: f64, degree: usize, knots: &[f64], n_basis: usize) -> Vec<f64> {
    ...
    for i in 0..(n_knots - 1) {
        ...
        if i == n_knots - 2 {
            prev[i] = if knots[i] <= x && x <= knots[i + 1] {
                1.0
            } else {
                0.0
            };
...
pub fn ns_basis(
    ...
    let n_basis = if include_intercept {
        df
    } else {
        df.saturating_sub(1).max(1)
    };
...
pub fn bs_names(var_name: &str, df: usize, include_intercept: bool) -> Vec<String> {
    ...
    let end = df;
    (start..end)
        .map(|i| format!("bs({}, {}/{})", var_name, i + 1, df))
        .collect()
}
...
pub fn ns_names(var_name: &str, df: usize, include_intercept: bool) -> Vec<String> {
    ...
    (start..end)
        .map(|i| format!("ns({}, {}/{})", var_name, i + 1, df))
        .collect()
}
...
pub fn is_names(var_name: &str, df: usize, increasing: bool) -> Vec<String> {
    ...
    (0..df)
        .map(|i| format!("ms({}, {}/{}, {})", var_name, i + 1, df, direction))
        .collect()
}
...
mod tests {
    ...
    fn test_bspline_non_negative() {
        ...
        for val in basis.iter() {
            assert!(
                *val >= -1e-10,
                "B-spline value should be non-negative, got {}",
                val
            );
        }
    }
    ...
    fn test_ispline_range() {
        ...
        for val in basis.iter() {
            ...
            assert!(
                *val <= 1.0 + 1e-10,
                "I-spline value should be <= 1, got {}",
                val
            );
        }
    }
