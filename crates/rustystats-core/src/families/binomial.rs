...
impl Family for BinomialFamily {
    ...
    fn unit_deviance_at(&self, yi: f64, mui: f64) -> f64 {
        ...
        // First term: y × log(y/μ). Only contributes when y > 0.
        if yi > 0.0 {
            if yi >= 1.0 - 1e-15 {
                dev += -mui_safe.ln();
            } else {
        ...
        // Second term: (1-y) × log((1-y)/(1-μ)). Only contributes when y < 1.
        if yi < 1.0 - 1e-15 {
    ...
    fn clamp_mu(&self, mu: &Array1<f64>) -> Array1<f64> {
        ...
        mu.mapv(|x| x.clamp(MU_MIN_PROBABILITY, MU_MAX_PROBABILITY))
    }
