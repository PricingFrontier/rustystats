...
def _validate_negbinomial_response(y: np.ndarray, name: str) -> None:
    ...
    if np.any(y < 0):
        n_neg = np.sum(y < 0)
        raise ValidationError(
            f"Negative Binomial family requires non-negative {name}. Found {n_neg} negative values."
        )
...
def validate_weights(
    ...
    if np.any(weights < 0):
        ...
        raise ValidationError(
            f"{name} contains {n_neg} negative values. Weights must be non-negative."
        )
...
def validate_offset(
    ...
    family_lower = family.lower()
    if is_exposure and family_lower in (
        "poisson",
        "quasipoisson",
        "gamma",
        "negbinomial",
