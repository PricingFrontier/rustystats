...
def _classify(name: str) -> dict:
    ...
    if m:
        return {
            "type": "spline",
            "spline_type": m.group(1),
            "variable": m.group(2),
            "basis_idx": int(m.group(3)),
            "basis_total": int(m.group(4)),
            "flags": m.group(5),
        }
...
class PMMLExporter:
    ...
    def _emit_interaction(self, feat: dict):
        comps = feat["components"]
        any(c["type"] == "categorical" for c in comps)
        any(c["type"] in ("linear", "constraint") for c in comps)
        any(c["type"] == "te" for c in comps)
        any(c["type"] == "fe" for c in comps)
    ...
    def _add_norm_continuous(self, name: str, source: str, x_grid: np.ndarray, effects: np.ndarray):
        df = ET.Element(
            "DerivedField",
            {
                "name": name,
    ...
    def _add_map_values(
        self, name: str, source: str, mapping: dict[str, float], default: float = 0.0
    ):
        df = ET.Element(
            "DerivedField",
            {
        ...
        mv = ET.SubElement(
            ...
            "MapValues",
            {
                "outputColumn": "encoded",
                "defaultValue": f"{default:.10g}",
            },
        )
        ET.SubElement(
            mv,
