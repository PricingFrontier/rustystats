...
class TestTargetEncodeBasic:
    ...
    def test_target_encode_prior_is_mean(self):
        ...
        _encoded, _name, prior, _stats = rs.target_encode(categories, target, "cat")
    ...
    def test_target_encode_level_stats_correct(self):
        ...
        _encoded, _name, _prior, stats = rs.target_encode(categories, target, "cat")
...
class TestTargetLeakage:
    ...
    def test_unique_categories_get_prior(self):
        ...
        encoded, _, prior, _ = rs.target_encode(
            categories, target, "cat", prior_weight=1.0, n_permutations=1, seed=42
        )
...
class TestMultiplePermutations:
    ...
    def test_more_permutations_reduces_variance(self):
        ...
        for seed in range(10):
            enc, _, _, _ = rs.target_encode(categories, target, "cat", n_permutations=1, seed=seed)
            variances_1perm.append(np.var(enc))

        # Run with 10 permutations multiple times
        variances_10perm = []
        for seed in range(10):
            enc, _, _, _ = rs.target_encode(categories, target, "cat", n_permutations=10, seed=seed)
...
class TestPriorWeight:
    ...
    def test_higher_prior_weight_shrinks_toward_mean(self):
        ...
        # Low prior weight
        _, _, _, stats_low = rs.target_encode(categories, target, "cat", prior_weight=0.1)

        # High prior weight
        _, _, _, stats_high = rs.target_encode(categories, target, "cat", prior_weight=10.0)
...
class TestEdgeCases:
    ...
    def test_single_category(self):
        ...
        encoded, _, _prior, stats = rs.target_encode(categories, target, "cat")
