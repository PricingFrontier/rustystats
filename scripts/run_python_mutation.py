#!/usr/bin/env python3
"""Run deterministic scored Python mutation targets.

This is intentionally bounded for CI. The target registry is small and reviewed;
each mutation has a focused test command that should kill it. Broader tool-based
mutation runs can be layered on top, but this script already produces a stable
machine-readable mutation score for critical Python guardrails.
"""
...
def _format_command(command: list[str]) -> list[str]:
    return [part.format(python=sys.executable) for part in command]
...
def _apply_mutation(target: dict, timeout: int) -> dict:
    ...
    command = _format_command(target["test_command"])
    started = time.time()
    ...
    return {
        ...
        "duration_seconds": round(time.time() - started, 3),
        "output_tail": "\n".join(mutant.stdout.splitlines()[-40:]),
    }
...
def main() -> int:
    ...
    targets = _load_targets("python", args.module)
    ...
    print(f"Python mutation score: {score:.2f}% ({killed}/{len(scored)} killed)")
